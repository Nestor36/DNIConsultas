...
async function consultar(a){
    ...
    const getapi = await fetch('api.php?dni='+document.getElementById("inputDNI").value.toLowerCase(), {
        method: 'GET',
        });
    ...
    if (a){
        if(rsp){
            if(GetInfo(rsp, '"rpta":', ',"') == 'false'){
                ...
                let dis = GetInfo(rsp, '"sexo":"', '","') == '1' ? 'MASCULINO' : 'FEMENINO';
                $("#GeneroPs").val(dis);
            }
        }
    } else {
        poupbox('GuxFiz - Dev Nestor', rsp);
    }
}
